...
public class Category {
    ...
    public Enumeration getAllAppenders() {
        return new Vector<Appender>(_appenders).elements();
    }

    public void callAppenders(LoggingEvent event) {

        // Delegate to custom appenders, if any:
        synchronized (_appenders) {
            for (Appender a : _appenders) {
                a.doAppend(event);
            }
        }

        // Also delegate to the slf4jLogger wrapped object:
        switch (event.level.toInt()) {
            case 25000:
                slf4jLogger.info(event.getMessage().toString());
                break;
            case Level.TRACE_INT:
                slf4jLogger.trace(event.getMessage().toString());
                break;
            case Level.DEBUG_INT:
                slf4jLogger.debug(event.getMessage().toString());
                break;
            case Level.INFO_INT:
                slf4jLogger.info(event.getMessage().toString());
                break;
            case Level.WARN_INT:
                slf4jLogger.warn(event.getMessage().toString());
                break;
            case Level.ERROR_INT:
                slf4jLogger.error(event.getMessage().toString());
                break;
    ...
    void differentiatedLog(Marker marker, String fqcn, int level, Object message, Throwable t) {
        ...
        if (locationAwareLogger != null) {
            ...
            if (level >= Event.EVENT.level) {
                slf4jLogger.info(marker, m); // This is the apixio event level.
            } else {
                switch (level) {
    ...
    private List<Appender> _appenders = new ArrayList<Appender>();
