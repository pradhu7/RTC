...
import com.apixio.util.nassembly.{ContactInfoUtils, NameUtils, SummaryUtils}

import java.util
import scala.collection.JavaConversions.asScalaBuffer
import scala.collection.JavaConverters._

object PatientActorUtils {

  def mergeClinicalActorSummaries(actors: List[PatientClinicalActorSummary]): Iterable[PatientClinicalActorSummary] = {
    actors.filterNot(e => e == null)
      .groupBy(_.getInternalId).map {
      case (_, duplicates) =>

        val baseActor = duplicates.maxBy(_.getBase.getDataCatalogMeta.getLastEditTime)
        ...
        // Set Meta
        val base = BaseConsolidator.mergeBaseSummaries(duplicates.map(_.getBase).asJava)
  ...
  def wrapAsPatient(clinicalActors: Seq[PatientClinicalActorSummary]): ClinicalActors = {
    def consolidateParsingDetails(actors: Seq[PatientClinicalActorSummary]): util.List[ParsingDetailsOuterClass.ParsingDetails] = {
      val parsingDetails = actors.flatMap(p => p.getBase.getParsingDetailsList)
      BaseConsolidator.dedupParsingDetails(parsingDetails.asJava)
    }

    def consolidateSources(actors: Seq[PatientClinicalActorSummary]): util.List[SourceOuterClass.Source] = {
      val sources = actors.flatMap(p => p.getBase.getSourcesList)
      BaseConsolidator.dedupSources(sources.asJava)
    }

    val caBuilder = ClinicalActors.newBuilder
    if (clinicalActors.isEmpty) {
      caBuilder.build
    }
    else {
      val parsingDetails = consolidateParsingDetails(clinicalActors)
      val sources = consolidateSources(clinicalActors)
      val catalogMeta = BaseConsolidator.mergeCatalogMeta(clinicalActors.map(_.getBase.getDataCatalogMeta).asJava)
        .toBuilder
        .setOriginalId(ExternalId.newBuilder().setAssignAuthority("Actors").build())
        .build()

      caBuilder.addAllClinicalActors(clinicalActors.map(SummaryUtils.normalizePatientClinicalActor).asJava)
      caBuilder.addAllParsingDetails(parsingDetails)
      caBuilder.addAllSources(sources)
      caBuilder.setDataCatalogMeta(catalogMeta)
      caBuilder.setPatientMeta(BaseConsolidator.mergePatientMeta(clinicalActors.map(_.getBase.getPatientMeta).asJava))
      caBuilder.build()
    }
