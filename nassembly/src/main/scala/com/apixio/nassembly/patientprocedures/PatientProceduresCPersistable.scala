...
class PatientProceduresCPersistable extends CPersistable[PatientProceduresExchange] {
  ...
  override def clusteringColumnsWithValues(exchange: PatientProceduresExchange): util.List[ColValueWithPersistentField] = {
    val cols: Array[AssemblySchema.AssemblyCol] = getSchema().getClusteringCols()
    val dateBucket = exchange.getProcedures.head.getBase.getDataCatalogMeta.getOriginalId.getId
