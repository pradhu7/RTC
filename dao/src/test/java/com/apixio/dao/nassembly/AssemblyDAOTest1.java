...
// @Ignore("Integration")
public class AssemblyDAOTest1
{
    private String assemblyTable = "cfvram1";
    private String partitionKey = "row";
    private String clusterKey = "dataBucket";
    private String nonClusterKey = "code";
    ...
    {
        String[] cols = {partitionKey, clusterKey, nonClusterKey, buffer};
        StatementGenerator statementGenerator;
        ...
        FieldValueList fieldValueList = new FieldValueListBuilder()
                ...
                .addLong(clusterKey, null)
                .addString(nonClusterKey, null)
                .addByteBuffer(buffer, null).build();

        cqlCrud.createTable(StatementGenerator.CREATE_TYPE.LEVELED, statementGenerator, assemblyTable, fieldValueList, partitionKey, clusterKey);
    }

    @Test
    public void TestWrite()
    {
        AssemblyDAO.Meta meta = new AssemblyDAO.Meta(database, assemblyTable, dataTypeTable, pdsId, "demo", 3600);

        String assemblyID = UUID.randomUUID().toString();
        byte[] serial = UUID.randomUUID().toString().getBytes(StandardCharsets.UTF_8);
        Map<String, Object> cluster = new HashMap() { {put(clusterKey, 25L);} };
        Map<String, Object> nonCluster = new HashMap() { {put(nonClusterKey, "someCode");} };
    ...
    {
        AssemblyDAO.Meta meta = new AssemblyDAO.Meta(database, assemblyTable, dataTypeTable, pdsId, "demo", 600);
