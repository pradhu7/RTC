                ...
                <div>
                		 ...
                		 <ul>
                		 		 <xsl:for-each select="n1:component/n1:structuredBody/n1:component/n1:section">
                		 		 	<xsl:if test="n1:templateId[1][@root != '2.16.840.1.113883.10.20.1.9']">		 
                		 		 		 <li><a href="#{generate-id(n1:title)}"><xsl:value-of select="n1:title"/></a></li>
                		 		 	</xsl:if>
                		 		 </xsl:for-each>
                		 </ul>
                </div>
		...
		<xsl:if test="n1:author">
			<xsl:for-each select="n1:author/n1:assignedAuthor">
				...
				<xsl:if test="n1:assignedPerson">
					<strong>Provider: <xsl:call-template name="show-name"><xsl:with-param name="name" select="n1:assignedPerson/n1:name"/></xsl:call-template></strong>
					<br />
				
				</xsl:if>				
				<xsl:if test="n1:assignedAuthoringDevice">
					<strong>Device: <xsl:call-template name="show-name"><xsl:with-param name="name" select="n1:assignedAuthoringDevice/n1:softwareName"/></xsl:call-template></strong>
					<br />
				</xsl:if>
				<xsl:if test="n1:representedOrganization">
					<br />
					<strong>Provider's Contact Information:</strong><br />
					<xsl:value-of select="n1:representedOrganization/n1:name" />
					<br />
					<xsl:apply-templates select="n1:representedOrganization/n1:addr" />
					<br />
					<xsl:apply-templates select="n1:representedOrganization/n1:telecom" />
					<br />
				</xsl:if>
			</xsl:for-each>
		</xsl:if>
